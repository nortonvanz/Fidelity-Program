# Clustering for Loyalty Program
...
Projeto de identificação do perfil de clientes mais valiosos para a empresa, para a construção de um programa de fidelidade.
...
Após o terceiro ano de empresa, atingindo a marca de 5000 clientes, o time de marketing percebeu que alguns deles compram produtos de ticket alto, com alta frequência, e contribuem de forma significativa no faturamento da empresa.

Tendo identificado esta oportunidade de aumentar o faturamento, decidiram criar um programa de fidelidade para estes clientes, chamado de "Loyals".
Logo, precisam que seja criada uma estrutura que identifique o perfil dos clientes mais valiosos, bem como dos demais grupos de clientes.
...
### 1.2 Objetivo
A partir dos dados de compras dos mais de 5000 clientes, o objetivo é agrupá-los com base na similaridade de seu perfil de consumo.
...
- O time de marketing precisa visualizar os perfis de cada grupo de clientes dentro ferramenta de visualização Metabase, já utilizada pela empresa.
...
**Planejamento de Infraestrutura Local e em Cloud:**
...
* Insight de negócio: Aplicar estratégias de marketing para oferecimento de produtos similares aos demais clusters, tomando como base os comprados pelos Loyals, visando reduzir a distância entre ambos neste quesito.

**H2 - Os clientes do cluster Loyals apresentam um número médio de devoluções 10% abaixo da média da base total de clientes.**

Falso: o cluster Insider tem um número médio de retornos 256% maior que a média da base total de clientes.

* Insight de negócio: Dado o número de devoluções bem acima do esperado, realizar levantamento detalhado de custos de logística reversa com área responsável, avaliando o impacto negativo frente às demais características positivas do Loyals.
...
* Insight de negócio: Destinar time dedicado da área de marketing especificamente para cuidar do relacionamento com os Loyals, dada representatividade do faturamento dentro da empresa, em relação ao negócio.
...
Os 4 modelos foram testados considerando K entre 2 e 12, visto que além disso, dificultaria muito o trabalho do time de marketing.

Estes testes foram realizados tanto no espaço de features (espaço original de dados), como no espaço de embedding (obtido por meio de algoritmos).

Os algoritmos utilizados para a criação do espaço de embedding foram: PCA, UMAP, t-SNE e um embedding baseado em árvore com Random Forest.
...
O melhor resultado encontrado foi com o modelo HC, com k=8, no espaço de embedding obtido com UMAP, com SS = 0.55. Desta forma, este foi o modelo eleito para deploy em produção.
...
O número de clientes Loyals esperado para o próximo ano será de: 1965.
...
Ações de marketing direcionadas para os demais grupos de clintes também podem ser realizadas, aumentando ainda mais o alcance do trabalho desenvlvido.  

# 9. Melhorias futuras
- Criar mais features a partir das já existentes, buscando gerar mais insumos para o aprendizado dos modelos.
- Utilizar ferramenta para gerenciamento de ambiente virtual mais eficiente, como o Poetry.


## 10 Referências
* O Dataset foi obtido no [Kaggle](https://www.kaggle.com/vik2012kvs/high-value-customers-identification).
* A imagem utilizada é de uso livre e foi obtida no [Pexels](https://www.pexels.com/pt-br/foto/brown-jest-for-you-box-264771/).
